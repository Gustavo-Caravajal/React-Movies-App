...
export const searchMedia = async (mediaType, query, page = 1) => {
    ...
    const res = await fetch(`${BASE_URL}/search/${mediaType}?query=${encodeURIComponent(query)}&language=en-US&page=${page}`, options);
...
export const getMovies = async (page = 1) => {
    const res = await fetch(`${BASE_URL}/discover/movie?include_adult=false&include_video=false&language=en-US&page=${page}&sort_by=popularity.desc`, options);
...
export const getSeries = async (page = 1) => {
    const res = await fetch(`${BASE_URL}/discover/tv?include_adult=false&include_null_first_air_dates=false&language=en-US&page=${page}&sort_by=popularity.desc`, options);
...
export const getMedia = async (mediaId, mediaType = "movie") => {
    if (!ALLOWED_MEDIA_TYPES.includes(mediaType)) {
        throw new Error("Invalid media type. Use 'movie' or 'tv'");
    }

    const res = await fetch(`${BASE_URL}/${mediaType}/${mediaId}?language=en-US`, options);
